    ...
    def converse_group(
        ...
        if (
            ...
            for persona in target_personas:
                ...
                report += f"{p.identity.name} consumed {agent_resource_num[p.agent_id]}% of unpolluted water. "
            current_conversation.append(
                (
                    PersonaIdentity("framework", "Mayor"),
                    (
                        f"Ladies and gentlemen, let me give you the monthly production report. {report}"
                    ),
                ),
            )
            ...
            if resource_limit is not None:
                p.store.store_thought(
                    (
                        ...
                        f" {resource_limit}% of unpolluted water to be used for production per factory owner."
                    ),
                    self.persona.current_time,
                    always_include=True,
                )
